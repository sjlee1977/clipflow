...
/**
 * DashScope Qwen (통义千问) LLM Library for Scene Splitting
 * Uses OpenAI-compatible endpoint for better international compatibility
 */
...
export async function splitScriptViaQwen(
  ...
  const characterInstruction = hasCharacter
    ? `\n**중요**: 이미지 생성 시 참조 이미지의 캐릭터 외모를 반드시 유지하세요. imagePrompt 앞에 "same character as reference image,"를 붙이세요.${subCharacterInstruction}`
    : subCharacterInstruction;
  ...
  const userPrompt = `입력된 대본을 정확히 ${sceneCount}개의 장면으로 나누어주세요.${characterInstruction}
...
${script}`;
  ...
  else if (isDeepSeek) effectiveModel = 'deepseek-v3';
  ...
  // OpenAI 호환 엔드포인트 사용 (국제/대만 계정: 항상 dashscope-intl)
  ...
  if (!res.ok) {
    ...
    throw new Error(`Qwen 장면 분할 실패 (${res.status}): ${err}`);
  }
