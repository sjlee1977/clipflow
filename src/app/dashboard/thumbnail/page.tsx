...
import { Wand2, Loader2, Download, RefreshCw, Copy, Check, ExternalLink } from 'lucide-react';
...
export default function ThumbnailPage() {
  ...
  async function handleGenerate() {
    if (!title.trim() && !customPrompt.trim()) return;
    setGenerating(true);
    setError('');
    setImages([]);
    try {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-5">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/8 bg-white/[0.02] p-4 space-y-4">
            ...
            {/* 제목 입력 */}
            <div>
              <p className="text-[10px] font-mono text-white/30 mb-1.5 uppercase tracking-wider">영상/포스트 제목</p>
              <input
                type="text"
                value={title}
                onChange={e => setTitle(e.target.value)}
                placeholder="예: 2026년 AI 툴 TOP 10 완벽 정리"
            ...
            <button
              ...
              disabled={generating || (!title.trim() && !customPrompt.trim())}
        ...
        <div className="space-y-4">
          {generating && (
            <div className="rounded-xl border border-white/8 bg-white/[0.02] flex flex-col items-center justify-center py-24 gap-4">
              ...
              <div className="text-center">
                <p className="text-[13px] font-bold text-white/60">AI가 썸네일을 생성하고 있습니다</p>
          ...
          {!generating && images.length === 0 && (
            <div className="rounded-xl border border-white/6 bg-white/[0.01] flex flex-col items-center justify-center py-24 gap-4">
              ...
              <p className="text-[12px] text-white/20 font-mono">제목을 입력하고 생성 버튼을 누르세요</p>
            </div>
          )}
