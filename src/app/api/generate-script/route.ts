...
const SCRIPT_WIKI_DIR = path.join(process.cwd(), 'wiki', 'script');

function readScriptWiki(relativePath: string): string {
  try {
    return fs.readFileSync(path.join(SCRIPT_WIKI_DIR, relativePath), 'utf-8');
  } catch {
    return '';
  }
}

function getLatestScriptFeedback(category: string): string {
  try {
    const feedbackDir = path.join(SCRIPT_WIKI_DIR, 'feedback', category);
...
// ── System Prompt Builder (Wiki 우선, 하드코딩 폴백) ──────────────────────────
...
function buildSystemPrompt(category: string, tone: string): string {
  ...
  // ── 심리학: 전용 full-prompt 우선 ──
  if (cat === 'psychology') {
    const wikiFullPrompt = readScriptWiki('psychology/full-prompt.md');
    const basePrompt = wikiFullPrompt || PSYCHOLOGY_SYSTEM_PROMPT;
    const principles = readScriptWiki('psychology/principles.md');
    const feedback = getLatestScriptFeedback('psychology');
    let extra = '';
    if (principles && !principles.includes('비워둠')) extra += `\n\n### 축적된 원칙\n${principles}`;
    if (feedback) extra += `\n\n### 최근 피드백 (반드시 반영)\n${feedback}`;
    return extra ? `${basePrompt}${extra}` : basePrompt;
  }

  // ── 정체성: wiki → 폴백 ──
  const wikiIdentity = readScriptWiki(`${cat}/identity.md`);
  const identity = wikiIdentity || (CATEGORY_IDENTITIES[cat] ?? CATEGORY_IDENTITIES.general);

  // ── 톤: wiki → 폴백 ──
  const wikiTone = readScriptWiki(`${cat}/tone.md`);
  const resolvedToneKey = CATEGORY_FIXED_TONES[cat] ?? tone ?? 'friendly_casual';
  const toneStyle = wikiTone || (TONE_STYLES[resolvedToneKey] ?? TONE_STYLES.friendly_casual);

  // ── 7단계 구조: wiki → 폴백(인라인) ──
  const wikiStructure = readScriptWiki('_shared/7-stage-structure.md');

  // ── 카테고리 단계 적응: wiki → 폴백 ──
  const wikiStageNotes = readScriptWiki(`${cat}/stage-notes.md`);
  const stageNote = wikiStageNotes || (CATEGORY_STAGE_NOTES[cat] ?? '');

  // ── 축적된 원칙 ──
  const principles = readScriptWiki(`${cat}/principles.md`);

  // ── 최신 피드백 ──
  const feedback = getLatestScriptFeedback(cat);

  // ── 공통 규칙 (wiki가 없으면 인라인) ──
  const commonRules = `▶ 말투 원칙
...
`;
  ...
  // 7단계 구조 (wiki 파일 or 인라인 하드코딩)
  if (wikiStructure) {
    prompt += wikiStructure;
  } else {
    prompt += `[1단계] 도입부 — 훅 & 몰입 유도
...
[7단계] 마무리 — 콜투액션 & 클로징`;
  ...
  // 카테고리 단계 적응
  if (stageNote && !stageNote.includes('기본 7단계 구조를 그대로 사용')) {
    prompt += `\n\n${stageNote}`;
  }
  ...
  // 축적된 원칙 (내용이 있을 때만)
  if (principles && !principles.includes('비워둠')) {
    prompt += `\n\n### 카테고리 축적 원칙\n${principles}`;
  }
