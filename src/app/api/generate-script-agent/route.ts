...
function getLatestFeedback(category: string): string {
  try {
...
function loadCategoryWiki(category: string): {
  ...
  stageNotes: string; principles: string; feedback: string;
} {
  const cat = category || 'general';
  return {
    identity:   readWiki(`${cat}/identity.md`),
    tone:       readWiki(`${cat}/tone.md`),
    structure:  readWiki('_shared/7-stage-structure.md'),
    stageNotes: readWiki(`${cat}/stage-notes.md`),
    principles: readWiki(`${cat}/principles.md`),
    feedback:   getLatestFeedback(cat),
  };
}
...
async function runDirector(
  ...
  topic: string, category: string, wiki: ReturnType<typeof loadCategoryWiki>,
  minLength: number,
...
점검 항목:
1. 첫 15초 훅이 시청자를 붙잡는가?
2. 각 단계 전환이 자연스러운가?
3. 클로징이 구독 CTA로 자연스럽게 연결되는가?
4. 길이가 ${minLength}자 이상인가?
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const res   = await runProducer(model, apiKeys, draft, minLength);
